...
public class QuestionsController : BaseController
{
    ...
    public async Task<ActionResult<List<QuestionDto>>> GetLessonQuestions(Guid lessonId)
    {
        ...
        var lesson = await _mediator.Send(new GetLessonQuery { Id = lessonId });
        if (lesson == null)
            return NotFound();
        ...
        var query = new GetLessonQuestionsQuery { LessonId = lessonId };
    ...
    public async Task<ActionResult<QuestionDto>> GetQuestion(Guid id)
    {
        ...
        // Get lesson to check access
        var lesson = await _mediator.Send(new GetLessonQuery { Id = question.LessonId });
        if (lesson == null)
            return NotFound();
    ...
    public async Task<ActionResult<Guid>> CreateQuestion(CreateQuestionCommand command)
    {
        ...
        var lesson = await _mediator.Send(new GetLessonQuery { Id = command.LessonId });
        if (lesson == null)
            return NotFound();
    ...
    [HttpPut("{id}")]
    [Authorize(Roles = "Instructor")]
    public async Task<IActionResult> UpdateQuestion(Guid id, UpdateQuestionCommand command)
    {
        if (id != command.Id)
            return BadRequest();
